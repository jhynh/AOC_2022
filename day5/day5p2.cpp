...
//similar to part 1 but the # moved keeps it's order instead of behaving one at a time
#include<iostream>
#include<fstream>
#include<vector>
#include<deque>
...
//print the top crates of each column
    for(int i=0; i<numCols;++i){
        cout << diagram[i].at(0);
    }
    cout << endl;
...
/*     for(int i=0; i<mv;++i){
    ...
    cout << mv << " " << frm << " " << to << endl;
    for(int i=mv; i>0; --i){
        cout << "MOVING: " << diagram[frm-1].at(i-1) << endl;
        diagram[to-1].push_front(diagram[frm-1].at(i-1));
    }
    for(int i=0; i<mv;++i){
        diagram[frm-1].pop_front();
    }
